...
CREATE TABLE "events" (
	...
	"description" text constraint events_description_length_ctr check (length(description) < 4096),
	"start" text constraint events_start_length_ctr check (length(start) < 26),
	"end" text constraint events_end_length_ctr check (length("end") < 26),
	...
	"lock" text not null constraint events_lock_length_ctr check (length(lock) < 64)
);
